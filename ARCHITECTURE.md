...
| `ppj_payments` | Pay-per-job payment tracking. Columns: `id`, `provider_id`, `request_id`, `amount_aed`, `stripe_payment_intent_id`, `status` (`pending`, `paid`, `failed` — CHECK constraint from migration 005 line 9), `accept_failed`, `created_at`. |
