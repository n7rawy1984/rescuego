...
export default function RatingForm({ jobId, providerId, onComplete }: Props) {
  const t = useTranslations('components.ratingForm')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (loading) return
    if (!stars) {
      setError(t('errors.selectStarRating'))
      return
    }
  ...
  if (submitted) {
    return (
      <div className="rounded-3xl border border-[#9FE1CB] bg-[#E1F5EE] px-5 py-8 text-center">
        ...
        <p className="font-semibold text-slate-950">{t('submittedTitle')}</p>
        <p className="mt-1 text-sm text-[#0F6E56]">{t('submittedDescription')}</p>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <div>
          <p className="mb-2 text-sm font-semibold text-slate-700">{t('ratingLabel')} <span className="text-red-500">*</span></p>
          <div className="flex justify-center gap-1 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:justify-start" role="group" aria-label={t('starRatingAriaLabel')}>
            {[1, 2, 3, 4, 5].map((star) => {
              ...
              return (
                <button
                  ...
                  aria-label={t('rateStarsAriaLabel', { count: star })}
          ...
          {stars > 0 && (
            <p className="mt-2 text-center text-sm font-medium text-slate-600 sm:text-start">
              {t(`ratingLabels.${stars}`)}
            </p>
          )}
        </div>
        <div className="flex flex-col gap-1.5">
          <label htmlFor="comment" className="text-sm font-semibold text-slate-700">{t('commentLabel')}</label>
          <textarea
            id="comment"
            value={comment}
