...
| **P4-H4 (Audit 4)** | Cron partial failures return HTTP 200 | Vercel sees success, no alerting. Silent cron failures go undetected. | **OPEN** |
...
| ID | Title | Status |
|---|---|---|
| M1 (Audit 1) | Admin API routes have no rate limiting | OPEN |
...
| P4-M2 (Audit 4) | `expireStaleQuotes` has no row LIMIT — can hold write locks on large batches | OPEN |
...
| F3-L4 (Audit 3) | Business plan `jobs_this_month` never resets — counter drifts upward indefinitely | OPEN |
