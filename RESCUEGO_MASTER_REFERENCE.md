...
**Migration baseline:** Migrations `001`–`042` are applied to the cloud database. `039_security_backstop.sql` (Batch 1), `040_rpc_integrity_state_safety.sql` (Batch 2), `041_admin_provider_status_atomic.sql` (Batch 3), and `042_fix_expire_stuck_phantom_column.sql` (Batch 3 runtime hotfix) are **deployed and runtime-verified**. `043_jobs_en_route_at_index.sql` (Batch 4 — P4-M1) exists in the repo and is **CODE COMPLETE — NEEDS RUNTIME VERIFICATION** (not yet applied). Next migration number: `044`.
...
> **Deployment note (Batch 3):** `supabase/migrations/041_admin_provider_status_atomic.sql` exists in the repo but is **NOT yet applied to the cloud database**; apply it after 040. It adds one narrow RPC, `admin_update_provider_status_atomic`, that performs the admin status/verified_badge change AND the `provider_kyc_log` insert in a single transaction (H5). Batch 3 also completed the **cron side of CRIT-02** in `ops/marketplace-cron/route.ts` (query widened to `status IN ('accepted','en_route','arrived')`, oldest-first by `requests.created_at`, LIMIT 50; the migration-040 RPC owns the thresholds) — so **CRIT-02 is now fully closed (RPC + cron)**. **Runtime hotfix (June 26):** the candidate query originally ordered by `requests.updated_at`, a phantom column — the cron threw `column requests.updated_at does not exist` and ran zero SLA releases. Fixed to order by `requests.created_at` (verified to exist; `updated_at` is never added to `requests` by any migration). **Related phantom-column risk — NOW FIXED in migration 042:** `expire_stuck_active_requests` (the weekly stuck-request cleanup) referenced the same phantom `r.updated_at` at `028:105` and `040:340` and would throw `column requests.updated_at does not exist` at runtime. New migration `042_fix_expire_stuck_phantom_column.sql` supersedes the deployed RPC via `CREATE OR REPLACE FUNCTION` with `r.updated_at` → `r.created_at` only (signature, `SECURITY DEFINER`, `SET search_path = public`, revoke/grant, and all Batch 2 LOW-03 logic preserved verbatim; deployed 028/040 untouched). A project-wide `updated_at` audit confirmed these were the ONLY two references binding to the `requests` table — all other matches are legitimate columns on `provider_locations`, `fair_price_config`, `stripe_events`, and `overage_payments`. Other Batch 3 closures (route-only, no migration): P4-C2, P4-H2, P4-H4, P4-M2 (`marketplace-cron` / `monthly-allowance-reset`), F3-L4 (business `jobs_this_month` now reset monthly), and M1 (rate limiting on `admin/providers/update`). The `admin_update_provider_status_atomic` RPC is **CODE COMPLETE — NEEDS RUNTIME VERIFICATION** until 041 is deployed. **Post-deploy QA to re-run:** the CRIT-02 end-to-end scenario (leave a job in `en_route` past 2h or `arrived` past 60m → the minute cron must auto-release it to `quoted`/`open` per D6) — this could not pass before this batch. See §6.3. **M1 follow-up:** `src/app/api/admin/sentry-verify/route.ts` rate limiting — **CLOSED in Batch 4**.

> **Deployment note (Batch 4) — rate limiting, GET hardening, lib hardening, index:** This is the last active security batch (Batch 5 is deferred/architectural). One DB change: `supabase/migrations/043_jobs_en_route_at_index.sql` adds a **partial index** `idx_jobs_en_route_at_active ON public.jobs (en_route_at) WHERE completed_at IS NULL` matching the admin stuck-job query (`admin/dashboard/page.tsx:80-85`) — **CODE COMPLETE — NEEDS RUNTIME VERIFICATION** until applied. Columns verified against `025_provider_state_machine.sql:19-21` (not phantom). **Findings closed (route/lib-only):**
> - **P4-H1** — `GET /api/requests` now rate-limited (`customer-active-request`, 60/min, SOFT); `GET /api/requests/quotes` was already limited (`customer-quotes`, 60/min — kept per ruling, not lowered to 30).
> - **HIGH-02** — the state-mutating expiry write in `GET /api/requests` (former lines 136-149) is removed; GET is now read-only. A `quoted` request older than 20 min is treated as expired in the response only; `marketplace-cron` owns DB expiry. Safe because writes are server-authoritative — `select_quote_atomic` re-validates live DB state and rejects with `request_not_in_quoted_status`/`quote_expired` (`customer/quote/select/route.ts:52-56`), so the brief read-vs-DB mismatch cannot cause an inconsistent write.
> - **HIGH-01** — `GET /api/requests/quotes` validates `request_id` as a UUID (`z.string().uuid()`) before any DB call; malformed input returns a uniform `400 { error: 'Invalid request' }`, no DB error leak.
> - **M1 (fully closed)** — `admin/sentry-verify` rate-limited (`admin-sentry-verify`, 30/min, SOFT), matching the Batch 3 `admin/providers/update` pattern. No admin route remains without rate limiting.
> - **P4-M4 / H4** — `lib/rate-limit.ts`: the per-instance `redisFallbackLogged` boolean (which silenced the Redis-unavailable warning for an instance's whole lifetime) is replaced with a 60s time-throttled log so fleet degradation stays visible.
> - **P4-M6** — `lib/ops-auth.ts`: Vercel's `CRON_SECRET` is now honored only if ≥32 chars (else ignored + logged `ops_route_weak_cron_secret`); secret comparisons use constant-time `timingSafeEqual`. Ops auth remains fail-closed (missing secret → 503, wrong → 401).
> - **P4-L4** — `lib/env.ts`: `OPS_CRON_SECRET` is now a **production-only hard-fail** in `validateEnv()` (throws at startup if missing in production), eliminating the silent runtime 503s. **Not required in local development** (cron is not exercised locally), so dev startup is unaffected. The existing ≥32-char check is retained.
>
> **Rate-limit helper signature change:** `checkRateLimitAsync(key, limit, windowMs, endpoint, mode: 'soft' | 'hard' = 'soft')` — an **optional 5th parameter** defaulting to `'soft'`. All 20 existing callers use the 4-arg form and are **fully backward compatible** (verified by tsc + lint + build). **HARD mode** (fail-closed when Redis is unavailable) is implemented as a capability for payment-critical routes but **intentionally NOT wired into any payment route in this batch** — deferred to a dedicated payment verification pass.
>
> **Post-deploy runtime verification:** (1) hammer `GET /api/requests` past 60/min as one customer → expect `429` + `Retry-After`; (2) confirm `GET /api/requests` performs no write (a stale `quoted` request shows as expired in the response while the row is untouched until cron runs); (3) after applying 043, run `SELECT indexname FROM pg_indexes WHERE tablename='jobs' AND indexname='idx_jobs_en_route_at_active';` → expect one row, and `EXPLAIN` the admin stuck-job query to confirm an index scan replaces the seq scan.
