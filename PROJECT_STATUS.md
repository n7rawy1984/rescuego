...
| Migration 058 | **APPLIED & RUNTIME-VERIFIED IN PRODUCTION (July 22, 2026), activation path.** `058_phase2_billing_period_first_activation.sql` — Phase 2 Billing-Period Integrity Part 1 (Q2/Q3): adds `providers.first_activation_at` marker column, a mandatory backfill (predicate `plan IN ('starter','pro','business') AND stripe_subscription_id IS NOT NULL AND first_activation_at IS NULL`, expected/matched 5 rows — the same 5 providers already corrected by the ad hoc Phase 1 SQL fix), extends migration 039's `enforce_providers_immutable_columns` to protect `first_activation_at`, and adds `initialize_first_subscription_atomic` (single atomic conditional `UPDATE ... WHERE first_activation_at IS NULL AND status = 'active'`; zeroes `jobs_this_month`, sets `jobs_reset_at`/`stripe_current_period_start`/`stripe_current_period_end`, never touches `job_credit_balance`; `service_role`-only grant, called exclusively from the webhook). Statement order in-file is ADD COLUMN → backfill UPDATE → trigger CREATE OR REPLACE, so the backfill runs before the column is protected. `npx tsc --noEmit` and `npm run lint` both exit 0. **Production runtime test (July 22, 2026):** new PPJ provider `f1d8fc11` completed one job (`jobs_this_month=1`), then upgraded to Starter via a real Stripe checkout. DB confirmed: `jobs_this_month=0`, `first_activation_at=2026-07-22 05:17:04` (written by the RPC), `stripe_current_period_start/end` and `jobs_reset_at` all written (`05:16:58` / `2026-08-22` / `05:16:58`), `job_credit_balance` untouched, dashboard showed 15 remaining (not 14) — confirms at runtime that the counter-zeroing/full-allowance behavior (Option B) and the marker write both work as designed. **One pre-existing legacy provider** (upgraded before this fix shipped) still shows 14 remaining — expected, not a bug; it self-corrects at its own next renewal once a fresh webhook delivery writes its real period dates (it was already marked initialized by the earlier Phase 1 manual backfill, so `initialize_first_subscription_atomic` correctly treats it as already-initialized and does not re-fire). |
| Migration 059 | **APPLIED IN PRODUCTION (July 22, 2026).** `059_negative_credit_balance_guard.sql` — Phase 2 Billing-Period Integrity Part 2: `CREATE OR REPLACE` of `select_quote_atomic` adding a `GREATEST(0, job_credit_balance - 1)` floor at the function's single credit-decrement site (defense-in-depth — the existing `job_credit_balance > 0` guard under the provider-row `FOR UPDATE` lock already made a negative value unreachable in practice). Grants restored exactly to the pre-059 baseline (`service_role`-only). `npx tsc --noEmit` and `npm run lint` both exit 0. **Not exercised by the July 22 runtime test** — that test only drove migration 058's initialization path (a subscription upgrade), which never calls `select_quote_atomic`. The floor guard itself is therefore still runtime-UNVERIFIED: applied to production, but no scenario has yet driven a provider through the at-limit/credit-consumption branch since it was applied. |
| Stripe webhook — billing-period root-cause fix | **CODE COMPLETE, RUNTIME-VERIFIED FOR ACTIVATION ONLY (July 22, 2026).** `src/app/api/stripe/webhook/route.ts`: new `resolveSubscriptionPeriod()` reads `current_period_start`/`current_period_end` off the single matched `Stripe.SubscriptionItem` — Stripe SDK v22 moved these fields off the top-level `Subscription` object, so the old top-level read always resolved `undefined`, silently writing NULL period dates on every activation and renewal. Zero/multiple matched items now throws (event fails, Stripe retries) instead of silently writing NULL. The renewal `updatePayload` object (route.ts:684-697) writes only `status`, `stripe_subscription_id`, `stripe_current_period_start`, `stripe_current_period_end` (plus `plan`/`job_credit_balance`/`last_upgrade_bonus_key` only inside the upgrade branch, route.ts:741-772) — it never writes `jobs_this_month` or `jobs_reset_at`; those two fields are written exclusively by `initialize_first_subscription_atomic` (once, at first activation) or the monthly-reset cron. The upgrade-bonus key (route.ts:749) now uses the guaranteed-non-null `currentPeriodStart` (previously `?? 'no-period'`, which degraded the key's ability to distinguish real billing periods). **Runtime-verified July 22, 2026 for the activation path only** (see Migration 058 row above: real, non-null period dates written, matching Stripe). **The renewal path is CODE COMPLETE but NOT YET RUNTIME-VERIFIED** — no real or Stripe Test-Clock renewal cycle has been run against a live already-initialized provider since this fix shipped. Required before the renewal path can be considered proven: a renewal-cycle test confirming fresh `stripe_current_period_start/end` are written without touching `jobs_this_month`, `jobs_reset_at`, or `first_activation_at`. |
| Next migration number | 060. Migrations 058 and 059 (rows above) are APPLIED to production (July 22, 2026). Migration 057 remains CODE COMPLETE — cloud-application status not reassessed this session (see §3; no new evidence was gathered on 057 while verifying the billing-period package) and covers Phase 3 Step 3 (Items C+D: D4 monthly-limit + credit-consumption fix) only. Migration 056 was an emergency grants-only hotfix, not part of Phase 3. Phase 3 Steps 4–5 (Item D5 restoration + abuse controls, 053 helper adoption) remain not yet written and are renumbered to 060+ per `TIERED_DISPATCH_051_ANALYSIS.md`. Named, mandatory follow-up: "Quote Reachability Parity" (GPS-staleness + radius/reachability parity between quote route and dashboard/053) — not yet scheduled to a migration number, must ship before Phase 3 is marked "server-side enforcement complete". A separate reconciliation migration is also needed to capture `expire_stale_open_requests`'s live definition into version control. |
...
| Last documented work session | July 22, 2026 (Phase 2 Billing-Period Integrity — migrations 058/059 applied to production; activation path/Option B/root-cause fix runtime-verified via a real Stripe checkout test; renewal path remains code-complete, not yet runtime-verified — see §6 "Billing-Period Integrity (Phase 2)"). Prior: July 15, 2026 (binding expiry-rule decision recorded; PPJ quote-display investigation closed per live production test; read-only Phase 3 readiness audit performed). |
...
| Fact | Value |
|---|---|
| Total migrations | 59 files exist in `supabase/migrations/` (001–059). 057 (Phase 3 Step 3 credit logic), 058, and 059 (Phase 2 Billing-Period Integrity) have been added since the prior count of 56. |
| Latest applied (cloud) | **VERIFIED: 054** (`054_phase3_step1_ssot_and_d5_infra.sql`, applied & runtime-verified July 12, 2026), **VERIFIED: 056** (`056_grants_hotfix.sql`, applied & runtime-verified July 13, 2026, grants-only, independent of 055), **VERIFIED: 058 and 059** (billing-period integrity, applied July 22, 2026 — 058's initialization/marker/backfill path confirmed by a live production test; 059's floor guard is applied but its specific branch has not yet been runtime-exercised — see §1 rows). **UNVERIFIED: 053, 055, and 057** — 053/055 are documented elsewhere as "code complete, not yet applied," but later session narratives (July 9 dashboard-wiring work assuming `get_nearby_open_requests` — migration 053 — was already callable in production) are inconsistent with "not applied"; this was not resolved this session (no direct Supabase access was used for 053/055/057's own application status) and must be checked live before any of the three is treated as applied or unapplied. |
| Next migration number | **060** (058/059 applied July 22, 2026; Phase 3 Steps 4–5 credit logic, per `TIERED_DISPATCH_051_ANALYSIS.md`) |
| Cloud state | Confirmed via explicit session verification: 001–050 (July 1 / July 5, 2026), 051–052 (July 8, 2026, corrected same day), 054 (July 12, 2026), 056 (July 13, 2026), 058 and 059 (July 22, 2026, per the live billing-period runtime test — see §1). **NOT independently reverified this session: 053, 055, 057** — recommended check before relying on any of the three: run `pg_get_functiondef('public.get_nearby_open_requests(...)'::regprocedure)` (053), `pg_get_functiondef('public.submit_quote_atomic(...)'::regprocedure)` (055, look for the Step 2b tier-delay gate), and `pg_get_functiondef('public.select_quote_atomic(...)'::regprocedure)` (057/059 — the live body already reflects 059's floor guard per the July 22 apply, but 057's own separate cloud-application milestone was never independently confirmed) directly against production. |
...
**Migrations 039–045 are the security and marketplace remediation batch. Migrations 051–057 are the Tiered Dispatch batch** (051 dispatch-foundation schema, 052 subscriber-count snapshot, 053 tiered visibility RPC, 054 Phase 3 Step 1 SSOT + D5 infra, 055 Phase 3 Step 2 visibility-delay gate in `submit_quote_atomic`, 056 grants hotfix, 057 Phase 3 Step 3 credit consumption). **Migrations 058–059 are a separate, independent batch — Phase 2 Billing-Period Integrity** (058 first-activation marker + backfill + initialization RPC, 059 negative-credit-balance floor guard in `select_quote_atomic`), applied and partially runtime-verified July 22, 2026 — see §1 and the dedicated §6 entry. 001–045 verified July 1, 2026; 046–050 verified July 5, 2026; 051–052 verified July 8, 2026; 054 verified July 12, 2026; 056 verified July 13, 2026; 058–059 verified/applied July 22, 2026. **053, 055, and 057 have no independent post-write cloud-application verification recorded — see correction note above.**

Complete migration sequence: 001–059. For architectural meaning of each migration see [ARCHITECTURE.md §2] (§2 table currently lists through 056 as of the last update — 057/058/059 additions still need to be recorded there, see `ARCHITECTURE.md` §2).
...
### Billing-Period Integrity (Phase 2) — Applied, Partially Runtime-Verified (July 22, 2026)

**Status: APPLIED to production. Activation path (root cause + Option B initialization + marker) is RUNTIME-VERIFIED. Renewal path is CODE COMPLETE but NOT YET RUNTIME-VERIFIED.** Not tracked as an LB-numbered blocker — the confirmed root cause is fixed and the highest-risk path (first activation) is proven; the remaining gap is a pending verification task, not a known defect.

**Root cause:** Stripe's SDK/API (npm `stripe` v22 / API 2025+) moved `current_period_start`/`current_period_end` off the top-level `Subscription` object onto each subscription item. `src/app/api/stripe/webhook/route.ts` previously read these fields off the top-level object, which always resolved `undefined` — `stripe_current_period_start`/`stripe_current_period_end` were silently written NULL on every activation and every renewal. Left unfixed, this would have permanently broken the monthly `jobs_this_month` reset once real billing periods rolled over (the reset cron has nothing correct to compare against).

**Phase 1 (prior, ad hoc):** the 5 already-subscribed providers affected by the NULL-write bug were corrected directly via an ad hoc SQL fix against production, before the durable code/migration fix below existed.

**Phase 2 (this batch — migrations 058/059 + webhook fix):**
1. `resolveSubscriptionPeriod()` (new, `route.ts`) reads the period off the single subscription item whose price matches `PLAN_BY_PRICE_ID`; zero/multiple matches throws instead of silently writing NULL.
2. `initialize_first_subscription_atomic` (migration 058) — Option B: a single atomic conditional `UPDATE` that runs exactly once per provider (`first_activation_at IS NULL AND status = 'active'`), zeroing `jobs_this_month` and setting `jobs_reset_at`/`stripe_current_period_start`/`stripe_current_period_end` to the real Stripe period — giving the provider their full monthly allowance from a clean baseline instead of inheriting whatever counter value existed pre-subscription. Never touches `job_credit_balance`.
3. `first_activation_at` marker column + backfill (migration 058) — makes "already initialized" durable so no future webhook delivery can re-zero the counter for a provider who has already been initialized once.
4. `GREATEST(0, job_credit_balance - 1)` floor (migration 059) in `select_quote_atomic` — defense-in-depth against a negative credit balance.
5. Upgrade-bonus key fix (`route.ts:749`) — now keyed off the guaranteed-non-null real period start instead of a degraded `'no-period'` fallback.

**Production runtime test (July 22, 2026):** new PPJ provider `f1d8fc11` completed one job, then upgraded to Starter via a real Stripe checkout. Confirmed in the database: `jobs_this_month=0`, `first_activation_at=2026-07-22 05:17:04`, `stripe_current_period_start/end` and `jobs_reset_at` all written correctly, `job_credit_balance` untouched; the provider's dashboard showed the full 15 remaining (not 14). This proves, at runtime: the webhook root-cause fix (real dates written), Option B first-subscription initialization (counter zeroed → full allowance), and the marker write — all for the **activation** path.

**Known, expected transitional artifact — not a bug:** one legacy provider who upgraded before this fix shipped still shows 14 remaining (one short of the full 15). It was already marked initialized by the Phase 1 manual backfill, so `initialize_first_subscription_atomic` correctly does not re-fire for it (by design — it only ever fires once, ever, per provider). It will self-correct automatically at its own next Stripe renewal, once a fresh webhook delivery writes its real, current period dates through the (now-fixed) renewal path.

**What remains unverified:** the **renewal path** — an already-initialized provider's subscription renewing (real elapsed billing period, or a Stripe Test Clock) — has not been exercised since this fix shipped. The July 22 test only exercised the activation/upgrade branch. Required to close this gap: run a real or Test-Clock renewal cycle against a live provider and confirm the renewal branch writes fresh `stripe_current_period_start/end` while leaving `jobs_this_month`, `jobs_reset_at`, and `first_activation_at` untouched (per the `updatePayload` object at `route.ts:684-697`, which by construction never includes those three fields). Also unverified: migration 059's `GREATEST(0, ...)` floor guard has not itself been exercised (no provider has been driven through the at-limit/credit-consumption branch since 059 was applied) — it remains an unproven defense-in-depth guard, not a confirmed-firing one.

**Out of scope for this batch (tracked separately):** a deeper, independent cancellation-attribution branching defect was noted during this work and is tracked in `DEFERRED_PRODUCT_BACKLOG.md`, not fixed here.
