...
**Phase 2 design — pending approval (July 8, 2026):** Full proposed `get_nearby_open_requests` redesign presented for review in-session (150km radius default, plan-aware CTE reading the caller's own `plan`/`visibility_reduced`, tier-delay CASE keyed on `providers_in_range_at_creation`/`subscribers_in_range_at_creation`, NULL-safe legacy handling that never hides a request, R1's frozen radius-expansion for `<10`-provider rows, full diff vs. 039, single-caller audit, three open questions flagged: the `<=10`/`<10` boundary mismatch between D1 and R1, whether the zero-subscriber fallback suppresses Q5's `visibility_reduced` penalty, and nationwide-vs-fixed-constant for the radius expansion). Not yet written to a migration file.
