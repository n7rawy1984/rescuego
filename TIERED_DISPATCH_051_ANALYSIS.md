...
**Dashboard-RPC wiring design — pending approval (July 9, 2026):** read-only design correcting the assumed premise (the dashboard already calls `get_nearby_open_requests` via the user-context client) and removing the actual bypass — a same-file fallback to a raw admin-client query whenever the RPC returns zero rows — presented for review; not yet implemented.
